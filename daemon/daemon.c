...
//#define TEST_INSTANCE_LOOP 1
#define MY_NAME "trdaemon"
...
getConfigDir( int argc, const char ** argv )
{
	...
	static char szConfigDir[PATH_MAX];
    const char * optarg;
    ...
    if( configDir == NULL )
        configDir = tr_getDefaultConfigDir( MY_NAME,szConfigDir );
...
int MAIN( int argc, char ** argv )
{
    ...
    if( pidfile_created )
        remove( pid_filename );
    tr_bencFree( &settings );
	tr_formatter_mem_uint();
	tr_formatter_size_uint();
	tr_formatter_speed_uint();
	tr_Done();
    return 0;
}
