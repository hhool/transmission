...
#ifndef REQUIRE_SESSION_ID
#define REQUIRE_SESSION_ID 0
#else
#define REQUIRE_SESSION_ID 1
#endif
               ...
               struct tr_rpc_server *  server )
{
    ...
    else
    {
		...
		char szParamValue[1024]={0};
		char szNameInTable[1024]={0};

		const bool paused = tr_getCGIParamValue(req->uri,"paused",szParamValue)?((strcmp(szParamValue,"true")==0)?true:false):false;
		tr_getCGIParamValue(req->uri,"nameintable",szNameInTable);

#if 0
		{
			int len;
			char*szDeccodeDisplayName = tr_base64_decode( szDisplayName, -1, &len );
			tr_free(szDeccodeDisplayName);
		}
#endif
		...
		for( i=0; i<n; ++i )
        {
			...
			tr_bencDictAddStr( args, "nameintable",szNameInTable);
...
#if 0
            fprintf( stderr, "compressed response is %.2f of original (raw==%u bytes; compressed==%u)\n",
...
handle_web_client( struct evhttp_request * req,
                   struct tr_rpc_server *  server )
{
	char  webClientDir[PATH_MAX]={0};
	
	tr_getWebClientDir( server->session,webClientDir );

    if( strlen(webClientDir) <= 0)
    {
