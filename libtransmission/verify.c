...
static tr_lock*
getVerifyLock( tr_session * ss )
{
    assert(ss && ss->verifylock);

    return ss->verifylock;
}

static void
verifyThreadFunc( void * vsession )
{
	tr_session* session = (tr_session*) vsession;
    ...
    for( ;; )
    {
        ...
        tr_lockLock( getVerifyLock(session) );
        stopCurrent = false;
        ...
        tr_free( node );
        tr_lockUnlock( getVerifyLock(session) );
...
tr_verifyAdd( tr_torrent * tor, tr_verify_done_cb verify_done_cb )
{
    ...
    tr_lockLock( getVerifyLock(tor->session) );
    tr_torrentSetVerifyState( tor, TR_VERIFY_WAIT );
    tr_list_insert_sorted( &verifyList, node, compareVerifyByPriorityAndSize );
    if( verifyThread == NULL )
        verifyThread = tr_threadNew( verifyThreadFunc, tor->session );
    tr_lockUnlock( getVerifyLock( tor->session) );
}
...
{
    tr_lock * lock = getVerifyLock(tor->session);
    tr_lockLock( lock );
...
void
tr_verifyClose( tr_session * session )
{
    tr_lockLock( getVerifyLock(session) );
