...
struct tau_tracker
{
    ...
    bool is_asking_dns;
	struct evdns_getaddrinfo_request * evdns_request;
...
{
	dbgmsg( t->key, "is_asking_dns %d", t->is_asking_dns );
    if( t->addr )
...
static void
tau_tracker_abort( struct tau_tracker * tracker )
{
	if(tracker->is_asking_dns && tracker->evdns_request)
	{
		evdns_getaddrinfo_cancel(tracker->evdns_request);
	}
}
static void
tau_tracker_upkeep( struct tau_tracker * tracker )
{
    ...
    if( !tracker->addr && !tracker->is_asking_dns )
    {
        ...
        tracker->evdns_request = evdns_getaddrinfo( tracker->session->evdns_base,
                           tracker->host, NULL, &hints,
                           tau_tracker_on_dns, tracker );
        return;
    }

	dbgmsg( tracker->key, "addr %p ipaddr %s -- connected %d (%u %u) -- connecting_at %u",
            tracker->addr,
			(tracker->addr!=NULL)?inet_ntoa(((struct sockaddr_in*)(tracker->addr->ai_addr))->sin_addr):"null",
        ...
        && ( !tracker->connecting_at ) )
    {
        ...
        dbgmsg( tracker->key, "Trying to connect. ipaddr %s Transaction ID is %u",
                inet_ntoa(((struct sockaddr_in*)(tracker->addr->ai_addr))->sin_addr),tracker->connection_transaction_id );
...
tau_handle_message( tr_session * session, const uint8_t * msg, size_t msglen )
{
    ...
    /*fprintf( stderr, "got an incoming udp message w/len %u\n", msglen );*/
