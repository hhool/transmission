...
static char *
getHomeDir( char szPath[PATH_MAX] )
{
    char * home = NULL;
...
static char *
getOldConfigDir( char szPath[PATH_MAX] )
{
    char * path = NULL;
#if !defined( WIN32 )
	char  home[PATH_MAX];
...
#ifdef SYS_DARWIN
    path = tr_buildPath( getHomeDir( home ), "Library",
                          "Application Support",
                          "Transmission", NULL );
#elif defined( WIN32 )
    char appdata[MAX_PATH]; /* SHGetFolderPath() requires MAX_PATH */
...
#else
    path = tr_buildPath( getHomeDir( home), ".transmission", NULL );
#endif
	if(path)
		strcpy(szPath,path);
...
static  char *
getOldTorrentsDir( char szPath[PATH_MAX] )
{
    char * path = NULL;

	char oldConfigDir[PATH_MAX];

	path = tr_buildPath( getOldConfigDir( oldConfigDir ), TORRENT_SUBDIR, NULL );
...
static  char *
getOldCacheDir( char szPath[PATH_MAX] )
{
    char * path = NULL;
#if !defined(WIN32)
	char  home[PATH_MAX];
...
#if defined( WIN32 )
    path = tr_buildPath( getOldConfigDir( oldConfigDir ), "Cache", NULL );
#elif defined( SYS_DARWIN )
    path = tr_buildPath( getHomeDir( home ), "Library", "Caches", "Transmission", NULL );
#else
    path = tr_buildPath( getOldConfigDir( oldConfigDir ), "cache", NULL );
#endif
	if(path)
...
migrateFiles( const tr_session * session )
{
	...
	char oldConfigDir[PATH_MAX];
    const bool should_migrate = strstr( getOldConfigDir( oldConfigDir ), ".transmission" ) != NULL;
...
const char*
tr_getDefaultConfigDir( const char * appname ,char szConfigDir[PATH_MAX])
{
    char * s = NULL;
#if !defined(WIN32)
	char  home[PATH_MAX];
    ...
    {
#ifdef SYS_DARWIN
        s = tr_buildPath( getHomeDir( home ), "Library", "Application Support",
                          appname, NULL );
#elif defined( WIN32 )
        ...
        if( ( s = getenv( "XDG_CONFIG_HOME" ) ) )
            ...
            s = tr_buildPath( getHomeDir( home ), ".config", appname, NULL );
#endif
    }
   
	if(s)
		strcpy(szConfigDir,s);
...
{
	static char user_dir[PATH_MAX]={0};
	char  home[PATH_MAX];
    ...
    if( strlen(user_dir) == 0 )
    {
        ...
        if( config_home && *config_home )
            ...
            config_file = tr_buildPath( getHomeDir( home), ".config", "user-dirs.dirs", NULL );
                ...
                if( end )
                {
                    *end = '\0';
                    ...
                    if( !memcmp( value, "$HOME/", 6 ) )
                        p = tr_buildPath( getHomeDir(home), value+6, NULL );
                    else
                        p = tr_strdup( value );
        ...
        if( strlen(user_dir)<=0 )
#ifdef __HAIKU__
            p = tr_buildPath( getHomeDir(home), "Desktop", NULL );
#else
            p = tr_buildPath( getHomeDir(home), "Downloads", NULL );
#endif
...
isWebClientDir( const char * path )
{
    ...
    tr_inf( _( "Searching for web interface file \"%s\" %d" ), tmp,ret);
    tr_free( tmp );
    return ret;
}
#if defined(TARGET_ANDROID)
static void tr_getPackageName(char PackageName[PATH_MAX])
{
	char buf[PATH_MAX]; 
	char* pStart = NULL;
	char* pEnd = NULL;
	...
	else
	{
		...
		tr_ninf( NULL, "path  \"%s\"", buf	 );
	} 
	pStart = buf+strlen("/data/data/");
	pEnd   = strstr(buf,"/files");
	if((pEnd-pStart)<=0)
	{
		return ;
	}
	memcpy(PackageName,pStart,pEnd-pStart);;
	PackageName[pEnd-pStart] = '\0';
}
#endif
const char *
tr_getWebClientDir( const tr_session * session UNUSED,char szDir[PATH_MAX] )
{
    char * s = NULL;
	char  home[PATH_MAX];
    ...
    else
    {
        ...
        if( tmp && *tmp )
            ...
            char * dhome = tr_buildPath( getHomeDir(home), ".local", "share", NULL );
        ...
        /* XDG_DATA_DIRS are the backup directories */
        {
			...
			char * fallback;
#if defined(TARGET_ANDROID)
			const char* PackageName = pkg;
			dbgmsg( "!!!tr_getPackageName %s",PackageName);
            fallback = tr_buildPath("/data/data/",PackageName,"/files",NULL);
			char * buf = tr_strdup_printf( "%s", fallback );
#else
            fallback = tr_buildPath("/usr/local/share:/usr/share",NULL);
	...
	tr_inf( _( "Searching for web interface file \"%s\"" ), szDir);
    return szDir;
}
