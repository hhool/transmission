...
blocklistLoad( tr_blocklist * b )
{
...

    {
        ...
        tr_inf( _( "Blocklist \"%s\" contains %u entries" ), base, b->ruleCount );
        tr_free( base );
    }
}
...
_tr_blocklistSetContent( tr_blocklist * b, const char * filename )
{
    ...
    if( fwrite( ranges, sizeof( struct tr_ipv4_range ), ranges_count, out ) != ranges_count )
        tr_err( _( "Couldn't save file \"%1s\": %2s" ), b->filename, tr_strerror( errno ) );
    else {
        char * base = tr_basename( b->filename );
        tr_inf( _( "Blocklist \"%s\" updated with %u entries" ), base, ranges_count );
        tr_free( base );
    }
