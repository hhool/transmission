...
void 
tr_ctorSetDisplayName(tr_ctor * ctor,const char* szNameInTable)
{
	if(ctor)
	{
		if(ctor->szNameInTable)
			tr_free(ctor->szNameInTable);

		ctor->szNameInTable = tr_strdup(szNameInTable);
	}
}
...
bool tr_ctorGetDisplayName(const tr_ctor * ctor,char** szNameInTable)
{
	...
	if(ctor->szNameInTable==NULL || strlen(ctor->szNameInTable) <= 0)
		return FALSE;

	(*szNameInTable) =ctor->szNameInTable;
