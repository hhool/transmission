...
tr_announcerInit( tr_session * session )
{
	...
	tr_inf( "new a->upkeepTimer %x",a->upkeepTimer);
...
tr_announcerClose( tr_session * session )
{
	...
	tr_inf( "free a->upkeepTimer %x",announcer->upkeepTimer);
...
on_scrape_error( tr_session * session, tr_tier * tier, const char * errmsg )
{
    ...
    /* schedule a rescrape */
    interval = getRetryInterval( tier->currentTracker );
    dbgmsg( tier, "Retrying scrape in %u seconds.", (size_t)interval );
    tr_torinf( tier->tor, "Retrying scrape in %u seconds.", (size_t)interval );
