          ...
          const char       * const key )
{
	...
	else if( tr_streq( key, keylen, "errorid" ) )
		tr_bencDictAddInt( d, key, st->errorid );
	...
	else if( tr_streq(key, keylen, "nameintable"))
		tr_bencDictAddStr( d, key, tor->szNameInTable);
				 ...
				 bool				bsync)
{
    ...
    *result = '\0';
    ...
    else /* successfully fetched the blocklist... */
    {
        ...
        if( fd < 0 )
            tr_snprintf( result, sizeof( result ), _( "Couldn't save file \"%s\": %s" ), filename, tr_strerror( errno ) );
					...
					bool			   bsync)
{
    ...
    dbgmsg( "torrentAdd: HTTP response code was %ld (%s); response length was %u bytes",
            ...
            struct tr_rpc_idle_data  * idle_data )
{
    ...
    else
    {
		...
		if( tr_bencDictFindStr( args_in, "nameintable", &str ) )
		{
			tr_ctorSetDisplayName(ctor,str);
			dbgmsg( "torrentAdd: nameintable is \"%s\"", str ? str : "(null)" );
		}
