...
{
    if( task->freebuf && task->response_inner)
...
task_finish_func( void * vtask)
{
    ...
    if( task->done_func != NULL )
        task->done_func( task->session,
                         ...
                         task->done_func_user_data,task->bSync);
                     ...
                     struct evbuffer    * buffer )
{
    ...
    if( web != NULL )
    {
		...
		if(buffer != NULL)
			buffer = buffer;
		task->response_inner = buffer ? FALSE: TRUE;
        task->freebuf = buffer ? NULL : task->response;

        tr_lockLock( web->taskLock );
        tr_list_append( &web->tasks_undone,task );
...
tr_webThreadFunc( void * vsession )
{
    ...
    for( ;; )
    {
        ...
        if( web->close_mode == TR_WEB_CLOSE_NOW )
            break;
        if( ( web->close_mode == TR_WEB_CLOSE_WHEN_IDLE ) && ( tr_list_size( web->tasks_undone ) == 0 ) )
            break;

        /* add tasks from the queue */
        tr_lockLock( web->taskLock );
        while( tr_list_size( web->tasks_undone ) > 0 )
        {
            /* pop the task */
            task = tr_list_pop_front( &web->tasks_undone);
            tr_list_append(&web->tasks_doing,task);
        ...
        while(( msg = curl_multi_info_read( multi, &unused )))
        {
            if(( msg->msg == CURLMSG_DONE ) && ( msg->easy_handle != NULL ))
            {
                ...
                curl_easy_cleanup( e );
				dbgmsg("removing a completed task.. taskCount is now %d (response code: %d, response len: %d)\n", taskCount, (int)task->code, (int)evbuffer_get_length(task->response) );
                tr_runInEventThread( task->session, task_finish_func, task );

				tr_list_remove_data(&web->tasks_doing,task);
    ...
    while( tr_list_size( web->tasks_undone ) > 0) {
		...
		task_finish_func(task);
    }

	while( tr_list_size( web->tasks_doing ) > 0) {
		task = tr_list_pop_front( &web->tasks_doing);
		task->bSync = 1;
		dbgmsg( "Discarding task \"%s\"", task->url );
		task_finish_func(task);
	}
