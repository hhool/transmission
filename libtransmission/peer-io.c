...
void datatype_pool_free()
{
	struct tr_datatype * datatype;
	while( datatype_pool != NULL )
	{
		datatype= datatype_pool;
		datatype_pool=datatype->next;
		tr_free(datatype);
	}
	datatype_pool = NULL;
}
...
utp_on_read(void *closure, const unsigned char *buf, size_t buflen)
{
    ...
    dbgmsg( io, "utp_on_read got %u bytes", buflen );
...
utp_on_write(void *closure, unsigned char *buf, size_t buflen)
{
    ...
    dbgmsg( io, "utp_on_write sending %u bytes... evbuffer_remove returned %d", buflen, rc );
...
utp_get_rb_size(void *closure)
{
    ...
    dbgmsg( io, "utp_get_rb_size is saying it's ready to read %u bytes", bytes );
...
utp_on_overhead(void *closure, bool send, size_t count, int type UNUSED)
{
    ...
    dbgmsg( io, "utp_on_overhead -- count is %u", count );
...
tr_peerIoTryWrite( tr_peerIo * io, size_t howmuch )
{
    ...
    dbgmsg( io, "in tr_peerIoTryWrite %u", howmuch );
...
tr_peerIoFlush( tr_peerIo  * io, tr_direction dir, size_t limit )
{
	...
	dbgmsg( io, "flushing peer-io, direction %s, limit %u, bytesUsed %d", (dir==TR_DOWN)?"TR_DOWN":"TR_UP", limit, bytesUsed );
    return bytesUsed;
}
