    ...
    if( m->piecesNeededCount == 0 )
    {
        ...
        if(( checksumPassed = !memcmp( sha1, tor->info.hash, SHA_DIGEST_LENGTH )))
        {
            ...
            if(( metainfoParsed = !err ))
            {
				...
				char * path=NULL;
                tr_benc newMetainfo;
                char * filename = path = tr_strdup( tor->info.torrent );
#ifdef WIN32
                ...
                if( !tr_bencLoadFile( &newMetainfo, TR_FMT_BENC, path ) )
                {
                    ...
                    {
                        tr_torrentSetLocalError( tor, TR_STAT_LOCAL_MAGNET_UNUSEABLE,"%s", _( "Magnet torrent's metadata is not usable" ) );
                        success = false;
                    }
