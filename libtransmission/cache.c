...
{
    tr_ptrArray blocks;
    int max_blocks;	   //max_bytes / (double)MAX_BLOCK_SIZE (16k)
    size_t max_bytes; //整个cache最大存储字节数, 默认TR_LIGHTWEIGHT 时为2M ，!TR_LIGHTWEIGHT 时为4M
...
tr_cacheFlushFile( tr_cache * cache, tr_torrent * torrent, tr_file_index_t i )
{
    ...
    dbgmsg( "flushing file %d from cache to disk: blocks [%u...%u]", (int)i, (size_t)first, (size_t)last );
