                  ...
                  size_t             buflen )
{
    ...
    ***/
    ...
    {
#ifdef WIN32
		char* filename = ConvertUtf8ToTChar(file->name); 
#else
		char* filename = file->name;
#endif

		tr_inf( "Before go to ioMode '%d'", ioMode );
		...
		tr_inf( "fd %d go to ioMode '%d'",fd, ioMode );
#ifdef WIN32
		tr_free(filename);
#endif
                  ...
                  size_t             buflen )
{
    ...
    while( buflen && !err )
    {
        ...
        if( ( err != 0 ) && (ioMode == TR_IO_WRITE ) && ( tor->error != TR_STAT_LOCAL_ERROR ) )
        {
            ...
            tr_torrentSetLocalError( tor, TR_STAT_LOCAL_FILE_ERROR,"%s (%s)",tr_strerror( err ), path );
            tr_free( path );
        }
    }
