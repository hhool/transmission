...
tr_sessionGetDefaultSettings( tr_benc * d )
{
    ...
    tr_bencDictAddBool( d, TR_PREFS_KEY_LPD_ENABLED,                     true );
#else
...
#if defined(TARGET_ANDROID)
	tr_bencDictAddInt ( d, TR_PREFS_KEY_DSPEED_KBps,                     4096 );
...
#if defined(TARGET_ANDROID)
	tr_bencDictAddBool( d, TR_PREFS_KEY_INCOMPLETE_DIR_ENABLED,          true );
#else
    tr_bencDictAddBool( d, TR_PREFS_KEY_INCOMPLETE_DIR_ENABLED,          false );
#endif
    tr_bencDictAddInt ( d, TR_PREFS_KEY_MSGLEVEL,                        TR_MSG_INF );
#if defined(TARGET_ANDROID)
	tr_bencDictAddInt ( d, TR_PREFS_KEY_DOWNLOAD_QUEUE_SIZE,             4 );
#else
	tr_bencDictAddBool( d, TR_PREFS_KEY_DOWNLOAD_QUEUE_SIZE,          	 5 );
#endif
...
#if defined(TARGET_ANDROID)
	tr_bencDictAddInt ( d, TR_PREFS_KEY_PREALLOCATION,                   TR_PREALLOCATE_NONE );
#else
    tr_bencDictAddInt ( d, TR_PREFS_KEY_PREALLOCATION,                   TR_PREALLOCATE_SPARSE );
#endif
#if defined(TARGET_ANDROID)
    tr_bencDictAddBool( d, TR_PREFS_KEY_PREFETCH_ENABLED,                false );
#else
...
#if defined(TARGET_ANDROID)
	tr_bencDictAddBool( d, TR_PREFS_KEY_SEED_QUEUE_ENABLED,              true );
#else
...
#if defined(TARGET_ANDROID)
	tr_bencDictAddInt ( d, TR_PREFS_KEY_USPEED_KBps,                     20 );
...
#if defined(TARGET_ANDROID)
	tr_bencDictAddBool( d, TR_PREFS_KEY_START,                           false );
#else
...
tr_sessionLoadSettings( tr_benc * d, const char * configDir, const char * appName )
{
    ...
    if( !configDir || !*configDir )
        configDir = tr_getDefaultConfigDir( appName ,szConfigDir);
...
#endif
		err = tr_bencLoadFile( &fileSettings, TR_FMT_JSON, path );
#ifdef WIN32
		tr_free(path);
#endif
		if( !err ) {
                        ...
                        const tr_benc * clientSettings )
{
    tr_benc settings;
	char * path=NULL;
    char * filename = path =  tr_buildPath( configDir, "settings.json", NULL );
#ifdef WIN32
	path =  ConvertUtf8ToTChar(filename);
#endif
    ...
    /* save the result */
    tr_bencToFile( &settings, TR_FMT_JSON, path );
...
onNowTimer( int foo UNUSED, short bar UNUSED, void * vsession )
{
    ...
    **/
    ...
    /* fprintf( stderr, "time %u sec, %u microsec\n", (size_t)tr_time(), (size_t)tv.tv_usec ); */
}
...
tr_sessionInitImpl( void * vdata )
{
	...
	tr_inf( "new session->nowTimer %x",session->nowTimer);
    ...
    **/
	...
	//定时保存下载的临时数据 onSaveTimer
    session->saveTimer = evtimer_new( session->event_base, onSaveTimer, session );
	tr_inf( "new session->saveTimer %x",session->saveTimer);
    tr_timerAdd( session->saveTimer, SAVE_INTERVAL_SECS, 0 );
...
sessionCloseImpl( void * vsession )
{
	...
	tr_inf( "free saveTimer %x",session->saveTimer );
    event_free( session->saveTimer );
    session->saveTimer = NULL;

	tr_inf( "free nowTimer %x",session->nowTimer );
...
static void
sessionDone( void * vsession )
{
	tr_session *  session = vsession;


	
	assert(tr_tracker_udp_is_idle( session ));
...
tr_sessionClose( tr_session * session )
{
    ...
    dbgmsg( "shutting down transmission session %p... now is %u, deadline is %u", session, (size_t)time(NULL), (size_t)deadline );

    /* close the session */
    tr_runInEventThread( session, sessionCloseImpl, session );
	while( true )
	{
		if(session->isClosed && tr_tracker_udp_is_idle(session))
			break;
		if(!tr_tracker_udp_is_idle(session))
		{
			tr_tracker_udp_abort(session);
		...
		dbgmsg( "waiting for the libtransmission udp tr_tracker_udp_is_idle" );
		tr_wait_msec( 100 );
	}
	tr_runInEventThread( session, sessionDone, session );
    ...
    {
        dbgmsg( "waiting on port unmap (%p) or announcer (%p)... now %u deadline %u",
    ...
    while( session->events != NULL )
    {
        ...
        dbgmsg( "waiting for libtransmission thread to finish... now %u deadline %u", (size_t)time(NULL), (size_t)deadline );
        tr_wait_msec( 500 );
    }
	...
	tr_lockFree( session->verifylock);
