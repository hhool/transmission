...
#define KEY_NAMEINTABLE			"nameintable"
...
loadDND( tr_benc * dict, tr_torrent * tor )
{
    ...
    else
    {
        tr_tordbg(
            tor,
            "Couldn't load DND flags. DND list (%p) has %u children; torrent has %d files",
...
tr_torrentSaveResume( tr_torrent * tor )
{
	...
	tr_bencDictAddStr( &top, KEY_NAMEINTABLE,tor->szNameInTable);
    savePeers( &top, tor );
    ...
    path =  filename = getResumeFilename( tor );
#ifdef WIN32
	path =  ConvertUtf8ToTChar(filename);
#endif
    if(( err = tr_bencToFile( &top, TR_FMT_BENC, path )))
        tr_torrentSetLocalError( tor, TR_STAT_LOCAL_RESUME_FILE_UNABLE_SAVE,"Unable to save resume file: %s", tr_strerror( err ) );
    tr_free( filename );
#ifdef WIN32
	tr_free(path);
#endif
...
loadFromFile( tr_torrent * tor, uint64_t fieldsToLoad )
{
	...
	path = ConvertUtf8ToTChar(filename);
#endif
    if( tr_bencLoadFile( &top, TR_FMT_BENC, path ) )
    {
        tr_tordbg( tor, "Couldn't read \"%s\"", path );
#ifdef WIN32
		tr_free(path);
#endif
        tr_free( filename );
    ...
    tr_tordbg( tor, "Read resume file \"%s\"", path );
	...
	if( ( fieldsToLoad & TR_FR_NAMEINTABLE )
		&& tr_bencDictFindStr( &top, KEY_NAMEINTABLE, &szNameInTable ) )
	{
		if(tor->szNameInTable)
			tr_free(tor->szNameInTable);
		tor->szNameInTable = tr_strdup(szNameInTable);

		fieldsLoaded |= TR_FR_NAMEINTABLE;
	}
