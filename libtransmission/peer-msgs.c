 ...
 *
 * $Id: peer-msgs.c 13154 2012-01-10 19:18:18Z jordan $
 */
...
{
    dbgmsg( msgs, "outMessage size is now %u", evbuffer_get_length( msgs->outMessages ) );
}
...
{
    struct peer_request req;
/*fprintf( stderr, "SENDING CANCEL MESSAGE FOR BLOCK %u\n\t\tFROM PEER %p ------------------------------------\n", (size_t)block, msgs->peer );*/
...
readBtId( tr_peermsgs * msgs, struct evbuffer * inbuf, size_t inlen )
{
    ...
    msgs->incoming.id = id;
    dbgmsg( msgs, "msgs->incoming.id is now %d; msgs->incoming.length is %u", id, (size_t)msgs->incoming.length );
             ...
             size_t           * setme_piece_bytes_read )
{
    ...
    else
    {
        ...
        *setme_piece_bytes_read += n;
        dbgmsg( msgs, "got %u bytes for block %u:%u->%u ... %d remain",
...
readBtMessage( tr_peermsgs * msgs, struct evbuffer * inbuf, size_t inlen )
{
    ...
    dbgmsg( msgs, "got BT addr %s, id %d, len %d, buffer size is %u",tr_address_to_string(tr_peerAddress(msgs->peer)),(int)id, (int)msglen, inlen );
...
canRead( tr_peerIo * io, void * vmsgs, size_t * piece )
{
    ...
    dbgmsg( msgs, "canRead: inlen is %u, msgs->state is %d", inlen, msgs->state );
...
fillOutputBuffer( tr_peermsgs * msgs, time_t now )
{
    ...
    **/
    ...
    {
        dbgmsg( msgs, "started an outMessages batch (length is %u)", evbuffer_get_length( msgs->outMessages ) );
    ...
    else if( haveMessages && ( ( now - msgs->outMessagesBatchedAt ) >= msgs->outMessagesBatchPeriod ) )
    {
        ...
        dbgmsg( msgs, "flushing outMessages... to %p peer addr %s (length is %u)", msgs->peer->io,tr_address_to_string(tr_peerAddress(msgs->peer)), len );
        ...
        && popNextRequest( msgs, &req ) )
    {
            ...
            && tr_cpPieceIsComplete( &msgs->torrent->completion, req.index ) )
        {
            ...
            if( !err && tr_torrentPieceNeedsCheck( msgs->torrent, req.index ) )
				...
				{
					tr_torrentSetLocalError( msgs->torrent,TR_STAT_LOCAL_PIECE_EROR, _( "Please Verify Local Data! Piece #%u is corrupt." ), (size_t)req.index );
					tr_inf("TR_STAT_LOCAL_PIECE_EROR Please Verify Local Data!!!!");
				}
...
sendBitfield( tr_peermsgs * msgs )
{
    ...
    dbgmsg( msgs, "sending bitfield... outMessage size is now %u", evbuffer_get_length( out ) );
    pokeBatchPeriod( msgs, IMMEDIATE_PRIORITY_INTERVAL_SECS );
...
sendPex( tr_peermsgs * msgs )
{
    if( msgs->peerSupportsPex && tr_torrentAllowsPex( msgs->torrent ) )
    {
        ...
        else
        {
            ...
            evbuffer_add_buffer( out, payload );
            pokeBatchPeriod( msgs, HIGH_PRIORITY_INTERVAL_SECS );
            dbgmsg( msgs, "sending a pex message; outMessage size is now %u", evbuffer_get_length( out ) );
            dbgOutMessageLen( msgs );
