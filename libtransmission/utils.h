...
/* #define DISABLE_GETTEXT */
 ...
 #undef _
 #define _( a ) ( a)//tr_strip_positional_args( a )
#endif
...
#define tr_torerr( tor, ... ) \
    do { char * name=NULL;\
        if( tr_msgLoggingIsActive( TR_MSG_ERR ) ) \
			name = ConvertUtf8ToTChar(tor->info.name); \
            tr_msg( __FILE__, __LINE__, TR_MSG_ERR, name, __VA_ARGS__ ); \
			tr_free(name); \
    } while( 0 )
...
#define tr_torinf( tor, ... ) \
    do { char * name=NULL;\
        if( tr_msgLoggingIsActive( TR_MSG_INF ) ) \
			name = ConvertUtf8ToTChar(tor->info.name); \
            tr_msg( __FILE__, __LINE__, TR_MSG_INF, name, __VA_ARGS__ ); \
			tr_free(name); \
    } while( 0 )
...
#define tr_tordbg( tor, ... ) \
    do { char * name=NULL;\
        if( tr_msgLoggingIsActive( TR_MSG_DBG ) ) \
		    name = ConvertUtf8ToTChar(tor->info.name); \
            tr_msg( __FILE__, __LINE__, TR_MSG_DBG, name, __VA_ARGS__ ); \
			tr_free(name); \
    } while( 0 )
#else
#define tr_torerr( tor, ... ) \
...
#undef  malloc
#undef  calloc
#define tr_malloc(x) _malloc_dbg(x,_NORMAL_BLOCK, __FILE__, __LINE__)
#define tr_malloc0(x) _calloc_dbg(1,x,_NORMAL_BLOCK, __FILE__, __LINE__)
#else
