...
typedef struct tr_torrent_peers
{
	...
	enum piece_sort_state      pieceSortStateNeed;
...
torrentNew( tr_peerMgr * manager, tr_torrent * tor )
{
	...
	t->pieceSortStateNeed = PIECES_SORTED_BY_INDEX;
...
{
	tr_inf( "free m->atomTimer %x,m->bandwidthTimer %x, m->rechokeTimer %x, m->refillUpkeepTimer %x",\
		m->atomTimer,\
		m->bandwidthTimer,\
		m->rechokeTimer,\
		m->refillUpkeepTimer);
...
pieceListSort( Torrent * t, enum piece_sort_state state )
{
	...
	tr_ndbg( " ", "piece_sort_state %d ",state);
...
pieceListRebuild( Torrent * t )
{
    ...
    if( !tr_torrentIsSeed( t->tor ) )
    {
        ...
        pieceListSort( t, t->pieceSortStateNeed );
...
pieceListResortPiece( Torrent * t, struct weighted_piece * p )
{
    ...
    if( t->pieceSortState != t->pieceSortStateNeed )
    {
       pieceListSort( t, t->pieceSortStateNeed);
       isSorted = true;
    }
        ...
        case TR_PEER_CLIENT_GOT_BLOCK:
        {
            ...
            else
            {
                ...
                if( tr_cpPieceIsComplete( &tor->completion, e->pieceIndex ) )
                {
                    ...
                    tordbg( t, "[LAZY] checked just-completed piece %u", (size_t)p );
...
ensureMgrTimersExist( struct tr_peerMgr * m )
{
	...
	tr_inf( "new  m->atomTimer %x,m->bandwidthTimer %x, m->rechokeTimer %x, m->refillUpkeepTimer %x",\
			m->atomTimer,\
			m->bandwidthTimer,\
				m->rechokeTimer,\
				m->refillUpkeepTimer);
...
bandwidthPulse( int foo UNUSED, short bar UNUSED, void * vmgr )
{
    ...
    while(( tor = tr_torrentNext( session, tor )))
    {
		...
		if(tor->errorid == TR_STAT_LOCAL_PIECE_EROR)
		{
			tr_inf("TR_STAT_LOCAL_PIECE_EROR Verify Local Data!!!!");
...
{
    dbgmsg("Starting an OUTGOING connection with %s - [%s] seedProbability==%d; %s, %s\n",
