...
void
tr_torrentSetLocalError( tr_torrent * tor,tr_stat_errtype_detail errid, const char * fmt, ... )
{
    va_list ap;
    ...
    switch( event->messageType )
    {
        case TR_TRACKER_PEERS:
        {
            ...
            const bool allAreSeeds = seedProbability == 100;
             ...
             if( allAreSeeds )
                tr_tordbg( tor, "Got %u seeds from tracker", event->pexCount );
            else
                tr_tordbg( tor, "Got %u peers from tracker", event->pexCount );
...
setLocalErrorIfFilesDisappeared( tr_torrent * tor )
{
    ...
    if( disappeared )
    {
        ...
        tr_torrentSetLocalError( tor, TR_STAT_LOCAL_DATA_UNFOUND,"%s", _( "No data found! Ensure your drives are connected or use \"Set Location\". To re-download, remove the torrent and re-add it." ) );
    }
...
torrentInit( tr_torrent * tor, const tr_ctor * ctor )
{
	...
	tor->isFilesInTorrentExsit = false;
	tor->isNewTorrent		   = true;
	...
	if( tr_ctorGetDisplayName(ctor,&szNameInTable))
		tor->szNameInTable = tr_strdup(szNameInTable);
    ...
    tor->isNewTorrent = stat( tor->info.torrent, &st );
    ...
    if( tr_ctorGetSave( ctor ) )
    {
        ...
        {
			int err = 0;
            char * path = tor->info.torrent;
#ifdef WIN32
			path =  ConvertUtf8ToTChar(tor->info.torrent);
#endif
            err = tr_bencToFile( val, TR_FMT_BENC, path );
            if( err )
                tr_torrentSetLocalError( tor, TR_STAT_LOCAL_TORRENT_FILE_UNABLE_SAVE,"Unable to save torrent file: %s", tr_strerror( err ) );
#ifdef WIN32
			tr_free(path);
#endif
...
{
    assert( tr_isTorrent( tor ) );
    assert( state==TR_VERIFY_NONE || state==TR_VERIFY_WAIT || state==TR_VERIFY_NOW || state == TR_VERIFY_CREATE_FILES_IN_TOR);
...
torrentGetActivity( const tr_torrent * tor )
{
	...
	if( tor->isCreating )
		return TR_STATUS_DOWNLOAD_CREATE_FILE_INTOR;
...
static void
torrentSureFileInTor( tr_torrent   * tor ,void (*CallBack)( tr_torrent*, bool),bool passby);
...
torrentStart( tr_torrent * tor, bool bypass_queue )
{
    switch( torrentGetActivity( tor ) )
    {
        ...
        case TR_STATUS_CHECK_WAIT:
            ...
            return;
            break;
		case TR_STATUS_DOWNLOAD_CREATE_FILE_INTOR:
			return;
			break;
	...
	if(tor->isNewTorrent||!tor->isSureFilesInTor)
	{
		torrentSureFileInTor(tor,torrentStart,bypass_queue);
		return;
	}
	if(tor->isSureFilesInTor && !tor->isFilesInTorrentExsit)
	{
		tor->isStopping = 1;
		return;
	}
	tor->isNewTorrent = false;
...
{
	tr_torrent   * tor;
	void (*CallBack)( tr_torrent*, bool);
	bool		 passby;
};

static void  SureFilesInTor( void * vdata )
{
	struct adding_data * data = vdata;
	...
	tr_tordbg( tor, "%s", "create files in torrent..." );
	...
	for( i = 0 ; i < info.fileCount ; i++)
	{
		...
		if( !tr_torrentFindFile2( tor, i, &base, &subpath, NULL ) )
		{
			const int prealloc = file->dnd || tor->session->preallocationMode;
			bool bEnable = false;
			char * part  = NULL;
			/* figure out where the file should go, so we can create it */
			base = tr_torrentGetCurrentDir( tor );
			bEnable = tr_sessionIsIncompleteFileNamingEnabled( tor->session );
			if(bEnable)
			{
#ifdef WIN32
				char* p = tr_torrentBuildPartial( tor, i );
				if(p)
				{
					subpath = ConvertUtf8ToTChar(p);
					tr_free(p);
				}
#else
				subpath = tr_torrentBuildPartial( tor, i );
#endif
			}
			else
			{
#ifdef WIN32
					subpath = ConvertUtf8ToTChar(file->name);
#else
		...
		if( !err )
		{
			...
			if((( fd = tr_fdFileCheckout( tor->session, tor->uniqueId, i,filename, doWrite,prealloc, file->length ))) < 0 )
			{
				err = errno;
				tr_torrentSetLocalError( tor, TR_STAT_LOCAL_FILE_ERROR,"%s (%s)",tr_strerror( err ), filename );
				break;
			}
			else if( doWrite )
			{
			...
			tr_inf( "out checking out '%s'", filename );
			tr_free( filename );
		}
		tr_free( subpath );
	}
	tor->isCreating = false;
	tor->isNewTorrent = false;
	tor->isSureFilesInTor = true;
	if(!err)
	{
		tor->isFilesInTorrentExsit = true;
	}

	data->CallBack(data->tor,data->passby);
	tr_free( data );
}
...
torrentSureFileInTor( tr_torrent   * tor ,void (*CallBack)( tr_torrent*, bool),bool passby)
{
	struct adding_data * data;

	assert( tr_isTorrent( tor ) );
	tor->isCreating = 1;

	data = tr_new0( struct adding_data, 1 );
	data->tor = tor;
	data->CallBack = CallBack;
	data->passby = passby;

	tr_threadNew( SureFilesInTor, data );
}
/**
***  Completeness
**/
...
tr_torrentCheckPiece( tr_torrent * tor, tr_piece_index_t pieceIndex )
{
    ...
    tr_deeplog_tor( tor, "[LAZY] tr_torrentCheckPiece tested piece %u, pass==%d", (size_t)pieceIndex, (int)pass );
                           ...
                           int                      trackerCount )
{
    ...
    if( ok )
    {
		char * path1=tor->info.torrent;
#ifdef WIN32
		path1 = ConvertUtf8ToTChar(tor->info.torrent);
#endif
		if(!tr_bencLoadFile( &metainfo, TR_FMT_BENC, path1 ))
		{
			bool hasInfo;
			tr_info tmpInfo;
								  ...
								  &hasInfo, &tor->infoDictLength ) )
			{
				...
				tr_metainfoFree( &tmpInfo );
				{
					...
					path =  ConvertUtf8ToTChar(tor->info.torrent);
#endif
					tr_bencToFile( &metainfo, TR_FMT_BENC, path );
#ifdef WIN32
					tr_free(path);
#endif
				}
...
deleteLocalData( tr_torrent * tor, tr_fileFunc func )
{
    ...
    ***/
	...
	if (remove(tmpdir) != 0) 
	{
		tr_dbg( "warning -- remove() error on %s: %s\n",
...
#ifdef WIN32
int remove_win32(const char * filename)
{
	bool ret = true;
	...
	if( !stat( filename, &sb ) && S_ISDIR( sb.st_mode ) )
		return RemoveDirectory( filename )?0:-1;
	else 
		ret = remove( filename );

	tr_inf("remove_win32 ret %d path %s",ret,filename);
...
#ifdef WIN32
			char* filename = ConvertUtf8ToTChar(f->name); 
#else
			char* filename = f->name;
#endif
            char * newpath = tr_buildPath( base, filename, NULL );
#ifdef WN32
			tr_free(filename);
#endif
                     ...
                     const char ** base, char ** subpath, time_t * mtime )
{
...
#ifdef WIN32
	name = ConvertUtf8ToTChar(file->name);
#else
	name = file->name;
#endif

    if( b == NULL ) {
		char * filename = tr_buildPath( tor->downloadDir, name, NULL );
    ...
    if( ( b == NULL ) && ( tor->incompleteDir != NULL ) ) {
        char * filename = tr_buildPath( tor->incompleteDir, name, NULL );
        if( fileExists( filename, mtime ) ) {
            b = tor->incompleteDir;
            s = name;
        }
    ...
    if( b == NULL )
	{
		char* p = tr_torrentBuildPartial( tor, fileNum );
#ifdef WIN32
		if(p)
		{
			part = ConvertUtf8ToTChar(p);
			tr_free(p);
		}
#else
		part = p;
