...
#include "torrent.h" /* tr_isTorrent() */
	...
	#define ltell	_telli64
  	typedef struct _stat64 sstat;
	#define ftruncate _ftruncate64
#else
	#define lstat stat
...
preallocate_file_sparse( int fd, uint64_t length )
{
	...
	tr_ndbg( "FILE", "Before preallocate_file_sparse need %d,length %lld",!success,length );
    if( !success ) /* fallback: the old-style seek-and-write */
	{		
		int64_t len = lseek( fd, length-1, SEEK_SET );
		success = ( len !=-1)
               && ( write( fd, &zero, 1 ) != -1 );
		if(success)
		{
			success=success && ( ftruncate( fd, length ) != -1 );
		}
		tr_ndbg( "FILE", "Finished preallocate_file_sparse need %lld,length %lld",length ,len);
	}
...
ssize_t
tr_pread( int fd, void *buf, size_t count, long long offset )
{
#ifdef HAVE_PREAD

	tr_ndbg( "FILE", "tr_pread fd %d,offset %lld ",fd,offset);

#if defined(TARGET_ANDROID) && defined(_LARGEFILE64_SOURCE)
...
#else
	const long long  lrc = lseek( fd, offset, SEEK_SET );
...
ssize_t
tr_pwrite( int fd, const void *buf, size_t count, long long offset )
{
#ifdef HAVE_PWRITE
	
	tr_ndbg( "FILE", "tr_pwrite fd %d,offset %lld ",fd,offset);
	...
	tr_ndbg( "FILE", " fd %d,offset %lld ",fd,offset);
...
int
tr_prefetch( int fd UNUSED, long long offset UNUSED, size_t count UNUSED )
{
#ifdef HAVE_POSIX_FADVISE
                  ...
                  uint64_t                 file_size )
{
    ...
    if( writable )
    {
        ...
        if( err ) {
            tr_err( _( "Couldn't create \"%1s\": %s" ), dir, tr_strerror( err ) );
            tr_free( dir );
            return err;
        }
        tr_free( dir );
    ...
    alreadyExisted = !lstat( filename, &sb ) && S_ISREG( sb.st_mode );

	tr_ndbg( "FILE", "Check cached_file_open writable %d alreadyExisted %d allocation %d",writable,alreadyExisted,allocation );

    if( writable && !alreadyExisted && ( allocation == TR_PREALLOCATE_FULL ) )
	{
		tr_ndbg( "FILE", "Before preallocate_file_full filename %s file_size %d allocation %d",filename,file_size,allocation );

		if( preallocate_file_full( filename, file_size ) )
            tr_dbg( "Preallocated file \"%s\"", filename );

		tr_ndbg( "FILE", "After preallocate_file_full filename %s file_size %d allocation %d",filename,file_size,allocation );
	}
    ...
    if( alreadyExisted && ( file_size < (uint64_t)sb.st_size ) )
    {
        ...
        {
            const int err = errno;
            tr_err( _( "Couldn't truncate \"%s\": %s" ), filename, tr_strerror( err ) );
            return err;
        }
    }
	tr_ndbg( "FILE", " writable  %d && !alreadyExisted %d && ( allocation == TR_PREALLOCATE_SPARSE ) %d ",writable,!alreadyExisted ,( allocation == TR_PREALLOCATE_SPARSE ));
    if( writable && !alreadyExisted && ( allocation == TR_PREALLOCATE_SPARSE ) )
        preallocate_file_sparse( o->fd, file_size );

	tr_ndbg( "FILE", " fd %d,filename %s ",o->fd,filename);
	...
	tr_ndbg( "FILE", " tr_set_file_for_single_pass ");
    tr_set_file_for_single_pass( o->fd );
	tr_ndbg( "FILE", " over tr_set_file_for_single_pass ");
    return 0;
}
