...
tr_msg_level __tr_message_level  = TR_MSG_DBG;
...
#define LOG_TAG "transmission-daemon"
#define LOGI(a)  __android_log_write(ANDROID_LOG_INFO,LOG_TAG,a)
static inline void OutputDebugString( const void * szBuf ) 
{
	LOGI(szBuf);
}
#elif defined(WIN32)
             ...
             size_t *     size )
{
    ...
    int fd;
    ssize_t n;
    const char * const err_fmt = _( "Couldn't read %s: %s" );
...
#pragma REMIND_ME("WARNING IMPLEMENT tr_mkdtemp")

char*
tr_mkdtemp( char * temppath )
{
#ifdef HAVE_MKDTEMP
    return mkdtemp( temppath );
#else
#if defined(WIN32)
    if( !_mktemp( temppath )
#else
    ...
    {
    	tr_ninf( NULL, "tr_mkdtemp \"%s\" failed", temppath );
	    return NULL;
	}
    return temppath;
#endif
           ...
           int          permissions )
{
                ...
                strchr( pp, TR_PATH_DELIMITER ) ) || ( p = strchr( pp, '\0' ) ) )
    {
		...
		tr_inf("stat path %s",path);

        if( stat( path, &sb ) )
        {
			tr_inf("tr_mkdir path %s",path);
            /* Folder doesn't exist yet */
            if( tr_mkdir( path, permissions ) )
            {
                const int err = errno;
                tr_err( _(
                           "tr_mkdir Couldn't create \"%1$s\": %2$s" ), path,
                       tr_strerror( err ) );
                tr_free( path );
                errno = err;
                return -1;
            }
			tr_inf("tr_mkdir path %s successed",path);
        }
        else if( ( sb.st_mode & S_IFMT ) != S_IFDIR )
        {
            /* Node exists but isn't a folder */
            char * buf = tr_strdup_printf( _( "File \"%s\" is in the way" ), path );
            tr_err( _( "!S_IFDIR Couldn't create \"%s\": %s,%s %d" ), path_in, buf,tr_strerror(errno),errno );
            tr_free( buf );
            tr_free( path );
            errno = ENOTDIR;
            return -1;
...
char* 
tr_getCGIParamValue(const char* URI,const char* szParm,char* szResult)
{
	const char* query;
	char szParamInner[1024]={0};
	char* szResultInner = NULL;
	char* szResultEnd = NULL;
	int nLen = 0;
	...
	sprintf(szParamInner,"%s=",szParm);

	szResultInner = strstr( query + 1, szParamInner );
	if(szResultInner == NULL)
		return NULL;
	
	szResultInner+=strlen(szParamInner);

	if((szResultEnd = strstr( szResultInner,"&"))&&szResultEnd)
	{
		nLen = szResultEnd-szResultInner;
		strncpy(szResult,szResultInner,nLen);
		szResult[nLen] = 0;
	}
	else
	{
		nLen = URI+strlen(URI)-szResultInner;
		strncpy(szResult,szResultInner,nLen);
		szResult[nLen] = 0;
	}
	return szResult;
...
tr_is_same_file( const char * filename1, const char * filename2 )
{
    struct stat sb1, sb2;
			...
			if(!stat( filename1, &sb1 ) && !stat( filename2, &sb2 ))
			{
				...
				res =  GetFileInformationByHandle( fh1, &fi1 ) && GetFileInformationByHandle( fh2, &fi2 ) 
					&& ( fi1.dwVolumeSerialNumber == fi2.dwVolumeSerialNumber ) && ( fi1.nFileIndexHigh == fi2.nFileIndexHigh ) 
...
static formatter_uinit(struct formatter_units * units)
{
	tr_free(units->units[TR_FMT_KB].name);
	tr_free(units->units[TR_FMT_MB].name);
	tr_free(units->units[TR_FMT_GB].name);
	tr_free(units->units[TR_FMT_TB].name);
}
static char*
...
void tr_formatter_size_uint()
{
	formatter_uinit( &size_units);
}
char*
...
void tr_formatter_speed_uint()
{
	formatter_uinit( &speed_units);
}
...
void tr_formatter_mem_uint()
{
	formatter_uinit( &mem_units);
}
char*
