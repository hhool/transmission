/**
 * Copyright 漏 Jordan Lee, Dave Perrett, Malcolm Jarvis and Bruno Bierbaumer
 *
...
Transmission.prototype =
{
	...
	****/
	...
	uploadTorrentFile: function(confirmed)
	{
		...
		// Submit the upload form
			...
			if ('' != $('#torrent_upload_url').val()) {
				remote.addTorrentByUrl($('#torrent_upload_url').val(), { paused: paused },{nameintable: 'nameintable-中文'});
			 } else
			 {
				args.url = '../upload?paused=' + paused +'&nameintable='+base64encode(utf16to8('中文'));
				args.type = 'POST';
	...
	promptToRemoveTorrents:function(torrents)
	{
		if (torrents.length === 1)
		{
			var torrent = torrents[0],
			    ...
			    message = '移除后需要重新加载Torrent的文件，确认移除?';
			dialog.confirm(header, message, '确认', 'transmission.removeTorrents', torrents);
		}
		else
		{
			var header = '移除 ' + torrents.length + ' 下载?',
			    message = '移除后需要重新加载Torrent的文件，确认移除?';
			dialog.confirm(header, message, '确认', 'transmission.removeTorrents', torrents);
		}
	},
	...
	promptToRemoveTorrentsAndData:function(torrents)
	{
		...
		{
			var torrent = torrents[0],
			    header = '移除 ' + torrent.getName() + ' 并删除下载数据?',
			    message = '移除后需要重新加载Torrent的文件，确认移除?';
			dialog.confirm(header, message, '确定', 'transmission.removeTorrentsAndData', torrents);
		}
		else
		{
			var header = '移除 ' + torrents.length + ' 下载并删除下载数据?',
			    message = '移除后需要重新加载Torrent的文件，确认移除?';
			dialog.confirm(header, message, '确定', 'transmission.removeTorrentsAndData', torrents);
		}
	},
	...
	***/
	...
	updateStatusbar: function()
	{
		...
		// visible torrents
		$('#filter-count').text( fmt.plural(this._rows.length, '任务') );
	},
	...
	updateFilterSelect: function()
	{
		...
		if (!this.filterTracker)
			str = '<option value="all" selected="selected">全部</option>';
		else
	...
	****/
	...
	onFilterTrackerClicked: function(ev)
	{
		...
		this.setFilterTracker(tracker==='全部' ? null : tracker);
	},

	setFilterTracker: function(domain)
	{
		// update which tracker is selected in the popup
		var key = domain ? this.getReadableDomain(domain) : '全部',
		    id = '#show-tracker-' + key;
