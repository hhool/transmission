    ...
    updateInfoPage = function () {
        ...
        if(torrents.length < 1)
            ...
            if( !haveUnverified && !leftUntilDone )
                ...
                str = ' 已下载 '+fmt.size(haveVerified) + ' 总量 ' + fmt.size(sizeWhenDone) + ' (' + str +'%)';
            else
                str = ' 已下载 '+fmt.size(haveVerified) + ' 总量 ' + fmt.size(sizeWhenDone) + ' (' + str +'%), ' + fmt.size(haveUnverified) + ' 未验证';
        }
        ...
        if(torrents.length < 1)
            ...
            if(!size)
                ...
                str = fmt.size(size) + ' (' + pieces.toStringWithCommas() + ' 块 @ ' + fmt.mem(pieceSize) + ')';
            else
                str = fmt.size(size) + ' (' + pieces.toStringWithCommas() + ' 块)';
        }
    ...
    updatePeersPage = function() {
        ...
        for (k=0; tor=torrents[k]; ++k)
        {
            ...
            html.push('<table class="peer_list">',
                   ...
                   '<th class="upCol">上行</th>',
                   '<th class="downCol">下行</th>',
    ...
    getAnnounceState = function(tracker) {
        ...
        switch (tracker.announceState) {
            ...
            case Torrent._TrackerWaiting:
                ...
                s = '下一次通知 ' + Transmission.fmt.timeInterval(timeUntilAnnounce);
                break;
            ...
            case Torrent._TrackerInactive:
                ...
                break;
            default:
                s = '未知站点状态: ' + tracker.announceState;
        }
        return s;
    },
