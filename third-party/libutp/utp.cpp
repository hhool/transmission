...
void UTPSocket::send_packet(OutgoingPacket *pkt)
{
	...
	size_t max_send = min<size_t>(max_window, opt_sndbuf, max_window_user);
...
{
	//Fixed by WangYao, 0 bytes will be writable
	if(0==to_write) 
		return true;
...
void UTPSocket::write_outgoing_packet(size_t payload, uint flags)
{
	...
	do {
		...
		if (payload && pkt && !pkt->transmissions && pkt->payload < packet_size) {
			...
			added = min<size_t>(payload + pkt->payload, max<size_t>(packet_size, pkt->payload)) - pkt->payload;
...
void UTPSocket::apply_ledbat_ccontrol(size_t bytes_acked, uint32 actual_delay, int64 min_rtt)
{
	...
	double window_factor = (double)min<size_t>(bytes_acked, max_window) / (double)max(max_window, bytes_acked);
	...
	assert(scaled_gain <= 1. + MAX_CWND_INCREASE_BYTES_PER_RTT * (int)min<size_t>(bytes_acked, max_window) / (double)max(max_window, bytes_acked));
...
size_t UTPSocket::get_packet_size()
{
	...
	if (DYNAMIC_PACKET_SIZE_ENABLED) {
		...
		return min<size_t>(mtu - header_size, max_packet_size);
	}
	else
	{
...
void UTP_Connect(UTPSocket *conn)
{
	...
	LOG_UTPV("0x%08x: Sending connect %s [%u]. conn_id_recv %d,seq_nr %d",
			 conn, addrfmt(conn->addr, addrbuf), conn->conn_id_recv,conn_seed,conn->seq_nr);
